...
<div class="min-h-screen bg-[#f7f7f7] flex items-center justify-center w-full">
    <div class="bg-white p-8 rounded-md shadow-lg w-full max-w-md">
        <div class="text-2xl font-semibold mb-6">{{ __('Edit Teacher') }}</div>

        <form method="POST" action="/dashboard/teachers/{{ $teacher->id }}" class="space-y-4"
            ...
            <div>
                ...
                @else
                <img src="{{ asset('assets/profile/usernull.jpg') }}" alt="{{ $teacher->name }}" class="img-preview">
                @endif
                ...
                <input
                    ...
                    id="gambar" type="file" value="{{ 'storage/' . $teacher->gambar }}" name="gambar"
