...
<div class="min-h-screen bg-[#f7f7f7] flex items-center justify-center w-full">
    <div class="bg-white p-8 rounded-md shadow-lg w-full max-w-md">
        <div class="text-2xl font-semibold mb-6">{{ __('Edit Mata Pelajaran') }}</div>

        <form method="POST" action="/dashboard/subjects/{{ $subject->id }}" class="space-y-4">
            @method('PUT')@extends('dashboard.index')
...
@endsection
            ...
            <div>
                ...
                <input id="name" type="text"
                    class="form-input mt-1 block w-full border border-gray-300 @error('name') border-red-500 @enderror"
                    name="name" value="{{ $subject->name }}" autocomplete="name" autofocus>
