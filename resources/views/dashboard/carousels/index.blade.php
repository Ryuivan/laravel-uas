...
<div class="container mx-auto px-5 md:px-10 xl:px-20 overflow-x-hidden">
    ...
    <h1 class="text-2xl mb-8 text-center font-semibold uppercase mt-16">Carousel</h1>
    ...
    <div class="flex flex-wrap justify-center space-x-5 md:justify-between">
        <a href="/dashboard/carousels/create"
            ...
            Add Carousel <i class="fa-solid fa-plus"></i>
        </a>
        <form action="/dashboard/carousel">
    ...
    <div class="flex justify-center items-center h-32">
        <span class="text-gray-400 font-medium">Carousel Not found</span>
    </div>
    @else
    ...
    <div class="flex justify-center flex-wrap">
        ...
        <div
            ...
            <img class="object-cover w-full rounded-t-lg h-56 md:h-48 md:w-48 md:rounded-none md:rounded-s-lg"
                src="{{ asset('storage/' . $carousel->gambar) }}" alt="{{ $carousel->name }}">
            @else
            <img class="object-cover w-full rounded-t-lg h-56 md:h-48 md:w-48 md:rounded-none md:rounded-s-lg"
                src="{{ asset('assets/profile/usernull.jpg') }}" alt="{{ $carousel->name }}">
            @endif
            <div class="flex flex-col w-52 justify-between p-4 leading-normal">
                <h5 class="mb-4 text-xl h-16 font-semibold tracking-tight text-gray-900">{{ $carousel->name }}</h5>
                <div class="flex space-x-2 justify-start">
                    <a href="/dashboard/carousels/{{ $carousel->id }}/edit"
                        class="transition duration-300 bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded-md">
                        <i class="fa-solid fa-edit"></i>
                    </a>
                    <form action="/dashboard/carousels/{{ $carousel->id }}" method="POST" class="">
                        @csrf
                        @method('DELETE')
                        <button type="submit" onclick="return confirm('Are you sure you want to delete this carousel?')"
