...
<div class="min-h-screen bg-[#f7f7f7] flex items-center justify-center w-full">
    <div class="bg-white p-8 rounded-md shadow-lg w-full max-w-md">
        <div class="text-2xl font-semibold mb-6">{{ __('Tambah Fasilitas') }}</div>

        <form method="POST" action="{{ route('fasilitas.store') }}" class="space-y-4" enctype="multipart/form-data">
            @csrf
