...
<div class="min-h-screen bg-[#f7f7f7] flex items-center justify-center w-full">
    <div class="bg-white p-8 rounded-md shadow-lg w-full max-w-md">
        <div class="text-2xl font-semibold mb-6">{{ __('Edit Gallery') }}</div>

        <form method="POST" action="{{ route('galleries.update', $gallery->id) }}" class="space-y-4"
            ...
            <div>
                ...
                <input id="name" type="text"
                    ...
                    name="name" value="{{ $gallery->name }}" required autocomplete="name" autofocus>
            ...
            <div>
                ...
                @if ($gallery->gambar)
                <img src="{{ asset('storage/' . $gallery->gambar) }}" alt="{{ $gallery->name }}"
                ...
                <input
                    ...
                    id="gambar" type="file" value="{{ 'storage/' . $gallery->gambar }}" name="gambar"
